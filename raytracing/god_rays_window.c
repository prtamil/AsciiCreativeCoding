...
#define FLOOR_AMBIENT_FAC                                                      \
  0.20f /* sky-ambient strength on the floor             */
...
/*
 * V3 — a point or direction in 3-D world space (camera, rays, sun, wall points
...
/*
 * RGB — a LINEAR (not gamma-encoded) radiance/colour triple, kept distinct from
 * V3 on purpose: these channels are LIGHT being added and multiplied along a
 * ray (HDR, may exceed 1.0 before tone mapping), not a position — the separate
 * type stops the two from being confused. Tone map + gamma happen once, in
 * paint_cell. Ref: Pharr, Jakob & Humphreys, "Physically Based Rendering"
 * (radiometry); Reinhard et al. (2002) tone mapping.
 */
typedef struct {
...
/* ── §9 LOGIC: hash RNG — window x-jitter (pure) ─────────────────── *
 *
...
/* ── §12 WORLDGEN + LOGIC: window/arch geometry — place_windows builds
 * g_windows (init/reseed); xy_in_any_window is pure — T6 ───────────────── *
 *
 * Three helpers:
 *
...
/*
 * Window — one pointed (lancet) arch cut into the wall plane: a rectangle
 * topped by two circular arcs meeting at a peak. The wall is opaque EXCEPT
 * inside these arches, so the windows ARE the light source — every god-ray
 * shaft is sunlight that passed through one. xy_in_any_window (§12.3) is the
 * point-in-arch test; the geometry there derives the two arc centres from
 * these four numbers.
 *   cx       centre x of the arch on the wall plane (m)
 *   cy_base  y of the sill — the rectangle's bottom edge (m)
 *   half_w   half-width; also the arc radius / 2 (sets the arc curvature)
 *   rect_h   height of the rectangular body below the arcs
 * Peak height above the sill = rect_h + √3·half_w (equilateral lancet, T6).
 * Ref: lancet / two-centred pointed-arch (Gothic) construction.
 */
typedef struct {
  float cx, cy_base, half_w, rect_h; /* arch placement + size — see doc above */
} Window;
...
/* ── §13 LOGIC: ray-wall intersection — slit-aware (pure)
 * ─────────────────────────── *
 *
...
/* ── §14 LOGIC: scene_hit — nearest of {wall, floor, sky} (pure)
 * ─────────────── */

/*
 * HitType / Hit — the eye-ray "hit record": which of the room's three surfaces
 ...
 * passes through a window slit and escapes outside). The wall is one uniform
 * plane (unlike a forest of distinct trunks), so no per-object index is needed.
 * Ref: ray-scene intersection record — Shirley, RTiOW.
 *   type  which surface: SKY (miss / through a window) / WALL / FLOOR
 *   t     ray parameter of the hit (distance along the unit ray)
 */
typedef enum { HIT_SKY = 0, HIT_WALL = 1, HIT_FLOOR = 2 } HitType;

typedef struct {
  HitType type; /* which surface: SKY (miss) / WALL / FLOOR             */
...
/* ── §15 LOGIC: sun visibility — slit-aware NEE (pure) — T5
 * ───────────────────── *
 *
...
/* ── §16 LOGIC: sky / sun radiance (pure)
 * ──────────────────────────────────────────── *
 *
...
static RGB shade_floor(V3 floor_point, V3 sun_dir, RGB sun_em,
  ...
  RGB ambient = rgb_scl(rgb_mul(horizon_col, albedo), FLOOR_AMBIENT_FAC);
  return rgb_add(direct, ambient);
}

/* ── §18 LOGIC: trace_ray (pure) — THE CORE, volumetric path tracer
 * ──────────────── *
 *
 ...
 * SEPARATE so the debug overlays (§21) can show each alone. Single-scattering
 ...
 */
/*
 * eye_march_distance — how far along the eye ray to integrate the mist. For a
 * surface hit it's the hit distance; for a SKY ray it's where the ray crosses
 * the window WALL plane (so we still gather the whole mist column up to the
 * wall), capped at FAR_CLIP. A tiny floor avoids a zero-length march.
 */
static float eye_march_distance(const Hit *hit, V3 ray_origin, V3 ray_dir) {
  float march_distance;
  if (hit->type == HIT_SKY) {
    if (ray_dir.z > 1e-6f) { /* heading toward the wall plane */
 ...
 * is dense and the sun is NOT blocked (next-event estimation), it adds
...
static RGB march_in_scatter(V3 ray_origin, V3 ray_dir, V3 sun_dir, RGB sun_em,
  ...
  for (int i = 0; i < MARCH_STEPS; i++) {
    ...
    if (extinction > EXTINCTION_EPS) {
      /* NEE: is the sun visible from this sample (not blocked by a wall)? */
...
/*
 * shade_hit — surface/sky radiance for whatever the eye ray hit: the dim wall,
 * the lit floor, or the sky seen through a window. The background term that the
 * in-scatter march sits in front of. Pure read of the hit record.
 */
static RGB shade_hit(const Hit *hit, V3 ray_origin, V3 ray_dir, V3 sun_dir,
  ...
  case HIT_WALL:
    return shade_wall();
  case HIT_FLOOR: {
    V3 floor_point = v3_add(ray_origin, v3_scl(ray_dir, hit->t));
    return shade_floor(floor_point, sun_dir, sun_em, horizon_col);
  }
  case HIT_SKY:
  default:
...
static Radiance trace_ray(V3 ray_origin, V3 ray_dir, V3 sun_dir, RGB sun_em,
                          RGB horizon_col) {
  /* what the eye ray hits, and how far to march through the mist. */
  Hit hit = scene_hit(ray_origin, ray_dir);
  float march_distance = eye_march_distance(&hit, ray_origin, ray_dir);
...
/* ── §19 SIMULATION: scene state + sun motion — scene_tick advances;
 * init/reseed are user events — T8 ──────────────────────────────── */

/*
 * Scene — the simulation's table of contents: WHAT is evolving (the sun clock
 * and the window-layout seed) and HOW the user drives / views it. Held by App
 * (§23); the lifecycle orchestrators scene_init / scene_reseed / scene_tick
 * take Scene*, while pure readers take only the fields they need.
 */
typedef struct {
  /* WHAT is simulated — evolving state */
  float time_secs; /* the sun clock; scene_tick advances it            */
  int seed;        /* RNG seed that determines the window layout        */
...
/* ── §20 LOGIC: camera — pinhole + pitch ray generation (pure) — T7
 * ──────────────────────────────── */

/*
 * Camera — a pinhole camera with PITCH: it can tilt up toward the high windows
 * (unlike the forest demo's level camera). camera_ray (§20) maps a screen cell
 * to a world ray, rotating it by the stored pitch — cos/sin are precomputed so
 * the per-pixel path needs no trig. fov_v is derived from fov_h and the cell
 * aspect so shapes aren't stretched by tall terminal cells.
...
typedef struct {
  ...
  float cos_pitch, sin_pitch; /* precomputed pitch rotation (tilt about X) */
...
/* ── §21 RENDER: screen + scene_draw → framebuffer → screen
 * ──────────────────── */
...
/* ── §23 APP / TICK — signals, input, the per-tick combine (see ARCHITECTURE)
 * ───────────────────────────────────── */
 ...
 * timers; no scene state LOGIC        §3-§9, §11-§18, §20,             nothing
 * — pure functions: vec/ xy_in_any_window (§12),          rgb math, blackbody,
 * tone map, scene_sun_dir (§19)              medium, phase, hashes, arch test,
 *                                                ray-wall/plane, scene_hit,
 * NEE, radiance, shading, trace_ray (the core), camera rays RENDER       §10,
 * §21, §22                    the SCREEN (ncurses cells) + the g_buf
 * framebuffer + g_have_256; reads scene, never writes it SIMULATION scene_tick
 * (§19)                 Scene.time_secs — the sun clock, the only PER-TICK
 * state. Worldgen place_windows (§12) writes g_windows, but only at init / `r`
 * reseed. DELAYS       Scene.paused gate (§19)          nothing — pause makes
 * scene_tick early-return; no holds/timers EFFECTS      — none — no stored
 * cosmetic state: the god rays / glow are integrated fresh every frame in
 ...
 *  mutate control/view state, g_windows (reseed), running / need_resize. They
 *  are NOT the tick; the only per-tick simulation write is scene_tick.
 * ════════════════════════════════════════════════════════════════════
 */
