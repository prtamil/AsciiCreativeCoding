...
A static still-life of a hand-tuned bonsai composition, with subtle wind rustle on the foliage.  Five classical bonsai styles cycleable with `n` (CHOKKAN / MOYOGI / SHAKAN / KENGAI / BUNJIN), six themes including a SUMI_E inverted ink-painting variant.

### Two-pass composition

```
PASS 1  (once per seed/style change):  generate skeleton + raster to char grid
PASS 2  (every frame):                 emit grid with per-cell rustle on foliage
```

The skeleton is generated by a recursive descent — trunk laid out as 6-30 segments along a style-specific path, each trunk node a branching opportunity, branches recursively subdividing.  Terminal recursion drops a foliage cloud.

### Style-specific trunk paths

| Style    | Trunk path |
|---|---|
| CHOKKAN  | `x = mid + tiny_noise(y)`  — vertical with wobble |
| MOYOGI   | `x = mid + amp · sin(2π · waves · frac)` — S-curve |
| SHAKAN   | `x = mid + slope · y` — straight slanted line |
| KENGAI   | Two-phase: short rise, then arc-and-cascade past pot rim |
| BUNJIN   | Nearly vertical, very tall, sparse branches |

### Aspect-corrected foliage clouds

Foliage at a branch tip is an ELLIPSE with radii `(rx, rx · CELL_ASPECT⁻¹)`, so it appears ROUND in physical pixels (terminal cells are 2× taller than wide; without correction the cloud looks like a vertically-squashed rugby ball).  Cells inside the ellipse get a random leaf glyph from the theme's leaf set; soft-edge falloff at 60 % radius gives clouds a fluffy outline rather than a hard-edged disc.

### Hash-gated wind rustle

Per frame, for every foliage cell:

```c
uint32_t h = hash3(col, row, ⌊time · RUSTLE_FREQ⌋);
if ((h & 0xFF) < 0xFF · RUSTLE_RATE)        /* ~5 % of cells */
    glyph = theme.leaf_set[(h >> 8) % leaf_count];
```

Stable per (col, row, time-bucket) → the same cell flickers at the same time-bucket boundary, creating a coherent rustle pattern instead of noise.  RUSTLE_FREQ = 6 Hz means rustle "ticks" advance 6 times per second; RUSTLE_RATE = 0.06 means about 6 % of foliage cells re-pick their glyph per tick.  Result: gentle twinkle, no boil.

### WINTER + SUMI_E special-cases

- **WINTER**: theme has `bare = true`; tree generation skips foliage clouds entirely; branch tips get a snow-tip glyph (`*`) instead.
- **SUMI_E**: theme has `inverted = true`; canvas pre-fills with white "paper" bg pair; A_BOLD/A_DIM disabled in the cell mapper.  Same recipe used in nuke.c, mandelbulb.c, sun_solar.c.

### Pot rendering

A trapezoidal pot with overhanging lip (`\___/` style) drawn LAST so any trunk segments that dipped into the pot region (KENGAI cascades) get cleanly overwritten.  All ASCII (`_`, `|`, `:`, `\\`, `/`); no ACS line-drawing chars.

*Files: `artistic/bonsai.c`*
...
A "sands of time" hourglass: curved quarter-elliptical bulbs (not the simple linear-tapered triangles of an earlier version), decorative wooden frame, falling-sand CA with per-cell MOMENTUM accumulation so neck-stream grains visibly accelerate, time-progress HUD, and AUTO-FLIP when the source bulb fully drains.

### Curved bulb shape

Each bulb's half-width as a function of vertical position uses a quarter-ellipse profile:
```
half_w(p) = NECK_HALF + (FULL_HALF − NECK_HALF) · √(1 − (1 − p)²)
```
where `p ∈ [0, 1]` is fractional distance from neck toward bulb cap.  At `p = 0` the bulb is at neck width; at `p = 1` it's at full width.  Curve is concave-out — the iconic "rounded hourglass" silhouette, not straight triangular taper.

`HG_FULL_FRAC = 0.16` (half-width as fraction of cols) keeps the container narrow — bulb-to-neck ratio ~8:1, matching real-life hourglass proportions.

### CA + per-cell momentum

Three parallel grids: `mask[r][c]` (cavity flag), `sand[r][c]` (colour layer index 0..16), `momentum[r][c]` (vertical speed accumulator 0..MOMENTUM_MAX).  Per tick, scan rows AWAY from gravity (bottom-to-top for `+1` gravity, top-to-bottom for `-1`) so each grain moves at most once per frame.  Movement priority: straight (toward gravity) > diagonal-left > diagonal-right.  Diagonals randomised per-cell-per-frame via `hash3(r, c, frame) & 1` to break tie-bias.

**Momentum bonus**: cells that successfully moved straight increment their `momentum` counter (capped at `MOMENTUM_MAX = 4`).  Cells with `mom ≥ 2` get an additional 1-2 cells of free-fall per tick.  Diagonal moves reset momentum to 0.  This produces the visible streaming acceleration through the narrow neck — a pure 1-cell-per-tick CA can't show acceleration; this hybrid does.

### Time-progress + auto-flip

`transferred` counts grains that crossed the neck midpoint in the current gravity direction.  HUD displays `Time: NN%` (`transferred / total_grains · 100`).  Two auto-flip triggers:

| Trigger | Condition |
|---|---|
| Full drain | `transferred ≥ total_grains` AND idle for ≥ AUTO_FLIP_SETTLE (~0.5s) |
| Stall fallback | idle for ≥ AUTO_FLIP_STALL (~3s) AND `transferred > 0` |

The fallback handles grains stuck in repose pockets above the neck; the flip's avalanche unsticks them.  Toggle with `a` key.

### Glyphs by momentum

Sand glyph picked by per-cell momentum: `@#` (resting, `A_BOLD`), `*+` (slow movement), `.,` (fast-falling, `A_DIM`).  Streaming neck visibly differs from settled piles: bright dense piles at top + bottom, sparse fast streak through the middle.

### Patterns + themes

**4 patterns** (cycle with `n`): NORMAL (8 layers), RAINBOW (16 thinner layers), GEOLOGICAL (8 layers with hash-jittered band thicknesses for organic strata), DUAL_FLOW (both bulbs filled at start, flipping moves sand the OTHER way without ever fully draining either).

**6 themes** (cycle with `t`): BEACH / VOLCANIC / ROYAL / ICE / ALIEN / NEGATIVE (white-paper-bg inverted, standard repo recipe).
...
**Per-pixel radial classifier:**
```
dx = col − cx
dy = (row − cy) · CELL_ASPECT     // round-disc correction
r  = √(dx² + dy²)

if r < R_disc:        SURFACE  (limb darkening · granulation · sunspots)
elif r < R_corona:    CORONA   (exponential exp(−d/falloff))
else:                 EMPTY
```
`R_disc` is `DISC_FRAC × min(cols, rows·CELL_ASPECT)` so the sun fits any terminal; `R_corona = R_disc · CORONA_MULT`.

**Surface shading** combines four physical effects in one expression:
```c
mu    = sqrt(1 − (r/R_disc)²)              // Eddington μ
tex   = fbm2d((dx − wind·t)·s, dy·s)       // animated granulation
tex_c = tex − 0.5
spot  = max(0, SPOT_THRESH − tex)²         // sunspot: where noise dips
spot  = spot * spot                         // sharpen spot cores
lum   = (LIMB_BASE + LIMB_BIAS · mu)
      * (1 − GRAN_AMP · tex_c·2 − SPOT_AMP · spot)
```
Eddington's 1-coefficient law `(1−u) + u·μ` gives the disc a dimmer edge than centre — without it the sun looks like a flat-coloured circle. Granulation drift in `x` is the animated boiling-cell illusion. Sunspots emerge naturally from low-noise regions of the same fBm — no separate spot system.

**Corona:**
```c
d   = r − R_disc
lum = CORONA_GAIN · exp(−d / (R_disc · CORONA_FALLOFF))
```
Exponential drop-off, no hard ring at the disc edge.

**Flare arcs in screen space** — no 3-D capsules, no Bézier polynomial roots. Each flare is a parabolic arc between two footpoints on the disc:
```
P(s) = (1 − s)·A + s·B  +  4s(1 − s) · (apex − chord_midpoint)
```
where `A`, `B` are footpoints sampled on the disc edge with constrained angular separation, and `apex_height ∈ [ARC_APEX_MIN, ARC_APEX_MAX]` is random per flare. Sampling 36 points along the arc and writing additively into a per-cell luminance buffer gives a smooth bright streak without any analytical distance computation.

**Lifecycle envelope** (rise → peak → fall):
```c
tau = age / lifetime              // ∈ [0, 1]
amp = 1 − |2·tau − 1|^1.4         // softer than triangle, sharper than parabola
```
Peaks at `tau = 0.5`, zero at the endpoints. Combined with `arc_amp = sin(π·s)` which peaks at the apex, the brightest cell of any flare is always `(apex, mid-life)`.

**Composition pipeline (3 passes per frame):**
1. **Disc + corona** → per-cell luminance buffer (~30 K cheap ops at 80×24)
2. **Flare overlay** → additively boost cells along each active flare's arc
3. **Emit** → ncurses cells with batched attron/attroff, NEGATIVE-theme white-bg pre-fill if active

**Themes (5):** SOLAR / BLUE_GIANT / RED_DWARF / ALIEN / NEGATIVE. Each is an 8-tier heat ramp; NEGATIVE follows the standard inverted-theme recipe (white bg, A_BOLD/A_DIM disabled).
...
Six-layer screen-space composition (sky, mountain silhouette, lava flows, crater glow, plume column, particles) over a single 1024-slot particle pool with four tagged types (BOMB / EMBER / ASH / SPARK).  Steady-state ~600-800 active particles per frame for a visually dense eruption.

**Mountain & flows** — startup work: stratovolcano silhouette generated as cone profile + 1-D fBm noise per column; crater carved as a cosine-weighted dip at the summit; lava-flow paths pre-traced from crater rim down both slopes once and rendered each frame as a distance-fading hot ramp.

**Plume** — 2-D fBm density column above the crater. Sample point drifts in x with `wind · time` and in noise's y with `rise · time`, so the column flows sideways AND boils upward as a coherent texture.  Cone falloff `(1 − dx²/half_w²)` keeps the plume from spreading across the whole sky.

**Single-pool particles, four types:**

| Type | Physics | Lifetime |
|---|---|---|
| BOMB | gravity + drag, ballistic arc, 4-position smoke trail | 2-4 s |
| EMBER | upward buoyancy + jitter + gentle wind | 3-6 s |
| ASH | mild buoyancy + stronger horizontal wind, slow grey settle | 6-12 s |
| SPARK | half-gravity, very fast outward jets at crater | 0.4-0.9 s |

**Four eruption patterns** (`n` cycles): STROMBOLIAN rhythmic moderate, VULCANIAN sparse + periodic 55-bomb burst every 4-7 s, PLINIAN continuous massive plume + heavy ash, HAWAIIAN strong fountain + bright lava flows + lighter ash.

**Random ambient bursts** — INDEPENDENT of pattern, every 8-22 s a dramatic surge fires: ~28 bombs + ~18 sparks + ~40 embers ejected simultaneously.  Random interval per occurrence so the timing is unpredictable; adds drama to even the calmest patterns (STROMBOLIAN's quiet phases, PLINIAN's steady plume).  This sits ALONGSIDE VULCANIAN's 55-bomb pattern-specific burst, not replacing it.

**Six themes** — DAY / DUSK / NIGHT / MARS / ASHFALL / NEGATIVE (white-paper-bg inverted, standard repo recipe).

**Composition order per frame**: white-paper pre-fill (NEGATIVE only) → sky vertical gradient → mountain silhouette filled solid → lava flows along slope paths → crater glow (3-cell pulsing strip) → plume column → particles (bomb trails first, then heads + non-bomb types).
