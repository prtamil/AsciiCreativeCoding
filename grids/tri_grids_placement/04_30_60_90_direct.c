...
/*
 * 04_30_60_90_direct.c — drop markers on a triangle grid whose triangles are
 * each split into six 30-60-90 wedges.
 *
 * Same as 01_equilateral_direct.c, but every triangle also shows its three
 * medians (corner to opposite mid-side). The medians are decoration: the
 * cursor still walks whole triangles and SPACE toggles a marker on the whole
 * triangle under it. All three medians meet at the centroid, which is exactly
 * where a marker lands.
 *
 * Sister files: grids/tri_grids/04_30_60_90.c (just the background),
 *               grids/tri_grids_placement/01_equilateral_direct.c (no medians).
...
typedef struct {
    ...
    double median_t;        /* a cell is on a median if it's within this of one */
...
/* the median char for a point by nearest of the triangle's 3 medians (each
 * corner to the opposite mid-side): '/', '\', '|'. out_min returns the distance
 * so the caller paints only cells within median_t. Line equations come from
 * grids/tri_grids/04_30_60_90.c §4. */
...
/* recipe step 2 — draw the grid: every cell -> its triangle -> the nearest
 * edge wins if it's close enough, else the nearest median, else blank.
 * Nothing stored; redrawn each frame. */
