...
static float mb_de(Vec3 pos, float power, int max_iter, float *out_smooth) {
  ...
  if (r < 1e-7f)
    return 0.0f;                  /* deep inside the set           */
...
static Mesh tessellate_mandelbulb(float power) {
  ...
  int *vidx =
      malloc((size_t)max_v * sizeof(int)); /* grid → vert idx, or -1 for miss */
  int nvert = 0, ntri = 0;
...
static void scene_build_shader(Scene *s) {
  switch (s->shade_idx) {
  case SH_PHONG_HUE:
    s->shader = (ShaderProgram){vert_mb, frag_phong_hue, &s->uni, &s->uni};
    break;
  case SH_NORMALS:
    s->shader = (ShaderProgram){vert_mb, frag_normals, &s->uni, &s->uni};
    break;
  case SH_DEPTH_HUE:
    s->shader = (ShaderProgram){vert_mb, frag_depth_hue, &s->uni, &s->uni};
    break;
  default:
    break;
  }
