...
static void render_gbuffer(const Mesh *meshes,
                           const Vec3 *albedos, const Vec3 *emissives,
                           ...
                           Mat4 view, Mat4 proj, int cols, int rows)
{
    ...
    for (int oi = 0; oi < n_objects; oi++) {
        ...
        rasterize_object(&meshes[oi], albedos[oi], emissives[oi],
                         mvp, models[oi], mv, nmat, cols, rows);
    }
}
    ...
    for (int r = 0; r < rows && r < GBUF_MAX_H; r++) {
        for (int c = 0; c < cols && c < GBUF_MAX_W; c++) {
            ...
            for (int i = 0; i < SSAO_SAMPLES; i++) {
                ...
                if (v3_dot(dir, N) < 0.f) dir = v3_neg(dir);  /* hemisphere */
...
static void render_lightpass(Vec3 cam_pos, bool ssao_enabled, int cols, int rows)
{
    ...
    for (int r = 0; r < rows && r < GBUF_MAX_H; r++) {
        for (int c = 0; c < cols && c < GBUF_MAX_W; c++) {
            ...
            g_light[r][c] = v3(amb.x + dif.x + sp.x + emit.x,
                               amb.y + dif.y + sp.y + emit.y,
...
static void hud_draw(const Scene *s, double fps)
{
    ...
    snprintf(status, sizeof status,
             " %5.1f fps  bloom:%s  ssao:%s  zoom:%.1f  tris:%d  %s ",
             fps,
             s->bloom_on ? "ON " : "OFF",
             s->ssao_on  ? "ON " : "OFF",
    ...
    mvprintw(hr + 1, 1,
             "bloom: threshold=%.2f  intensity=%.2f  kernel=%d-tap Gaussian (separable)",
...
static bool app_handle_key(App *app, int ch)
{
    ...
    case 'a': case 'A': s->ssao_on  = !s->ssao_on;  break;
    case '=': case '+':
