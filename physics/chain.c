...
static char slope_glyph(int dx, int dy)
{
    ...
    if      (adx >= 2 * ady)  return '-';
    else if (ady >= 2 * adx)  return '|';
    else if (dx * dy > 0)     return '\\';
    else                      return '/';
}
...
static void project_link_constraint(ChainNode *a, ChainNode *b, float rest)
{
    ...
    if (a->pinned && b->pinned) {
        ...
        b->x -= cx; b->y -= cy;            /* b absorbs full correction */
    } else if (b->pinned) {
        a->x += cx; a->y += cy;            /* a absorbs full correction */
    } else {
        a->x += 0.5f * cx; a->y += 0.5f * cy;   /* split 50/50 */
...
static void relax_link_constraints(Chain *c)
{
    for (int iter = 0; iter < c->n_iter; iter++) {
        for (int i = 0; i < c->n_nodes - 1; i++)
            project_link_constraint(&c->nodes[i], &c->nodes[i + 1],
                                    c->link_rest);
    }
}
 ...
 */
static void drive_wave_anchor_horizontal(Chain *c, int sub_step,
                                          float dt, float sub_dt)
{
 ...
 * of the screen.
 */
static void drive_wave_anchor_vertical(Chain *c, int sub_step,
                                        float dt, float sub_dt)
{
    float sub_t      = (float)sub_step / (float)SUB_STEPS;
    float phase_now  = c->wave_phase
 ...
 */
static void apply_preset_driver(Chain *c, int sub_step,
                                 float dt, float sub_dt)
{
    if      (c->preset == 3) drive_wave_anchor_horizontal(c, sub_step, dt, sub_dt);
    else if (c->preset == 5) drive_wave_anchor_vertical  (c, sub_step, dt, sub_dt);
}
...
{
    if (strain < 0.04f) return CP_LINK_LO;
    if (strain < 0.12f) return CP_LINK_MID;
    return CP_LINK_HI;
}
...
static void scene_init(int preset)
{
    ...
    switch (preset) {
    case 0: preset_hanging  (&g_scene.chain); break;
    case 1: preset_pendulum (&g_scene.chain); break;
    case 2: preset_bridge   (&g_scene.chain); break;
    case 3: preset_wave     (&g_scene.chain); break;
    case 4: preset_festoon  (&g_scene.chain); break;
    case 5: preset_snake    (&g_scene.chain); break;
    case 6: preset_storm    (&g_scene.chain); break;
    case 7: preset_slingshot(&g_scene.chain); break;
    case 8: preset_flag     (&g_scene.chain); break;
    case 9: preset_double   (&g_scene.chain); break;
    }
}
...
static void draw_hud_bottom(void)
{
    const char *hint_full =
        " q:quit  p:pause  r:reset  n/N:preset  t/T:theme  "
        "+/-:iter  w:wind  ]/[:fps ";
    const char *hint_short =
        " q:quit  p:pause  r:reset  n:preset  t:theme ";
...
int main(void)
{
    ...
    while (!g_quit_flag) {
        ...
        while ((ch = getch()) != ERR) {
            ...
            case '+': case '=':
                if (g_scene.chain.n_iter < N_ITER_MAX) g_scene.chain.n_iter += 5;
                break;

            case '-':
                if (g_scene.chain.n_iter > N_ITER_MIN) g_scene.chain.n_iter -= 5;
                break;
            ...
            case ']':
                if (g_scene.sim_fps < SIM_FPS_MAX) g_scene.sim_fps += SIM_FPS_STEP;
                break;

            case '[':
                if (g_scene.sim_fps > SIM_FPS_MIN) g_scene.sim_fps -= SIM_FPS_STEP;
                break;
            }
        }
