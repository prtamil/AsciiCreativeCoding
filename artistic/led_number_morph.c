 ...
 * CORE IDEA
 * ─────────
 * The 168 dots never wander between segments. Each one is permanently
 * branded as "I belong to segment B" (or A, C, …, G). When the current
 * digit lights B, that particle springs to a fixed slot on B; when B is
 * dark, the particle just drifts lazily toward digit-centre. The morph is
 * not 168 dots solving a global assignment — it is 7 independent
 * spring-clouds turning on or off in lockstep.
 *
 * HOW TO THINK ABOUT IT
 * ─────────────────────
 * Picture seven LED bars, each strung with 24 beads on an elastic. When the
 * bar is "lit", the elastic snaps tight and the beads line up evenly along
 * it. When the bar goes "dark", the elastic goes slack and the beads slump
 * toward the centre of the digit, jiggling. The whole 7-segment display is
 * just seven of these elastic strings sharing a centre point.
 *
 * ALGORITHM IN STEPS
 * ──────────────────
 *  1. Encode each digit 0-9 as a 7-bit segment mask (k_segs[]).
 *  2. Bind particle i permanently to segment s = i / 24.
 *  3. On digit change, for each segment, if its bit is set, lay 24 evenly
 *     spaced targets (tx,ty) along the segment's line; else mark idle.
 *  4. Per frame:
 *       - Active particle: F = SPRING_K·(target − pos), velocity damped by
 *         DAMP. Critically damped settle to target with no overshoot.
 *       - Idle particle: weak spring toward (cx,cy) with JITTER nudge.
 *  5. Render: if active and within 0.8 cells of target, draw '─' or '│'
 *     based on segment orientation and digit's bright colour; further out
 *     draw '+' or '.' in the in-flight pair. Idle particles only render
 *     while moving (speed² > 0.04) so the field looks calm at rest.
 *
 * KEY FORMULAS
 * ────────────
 *  F = k·(target − x)            Hooke spring force (per axis)
 *  v += (k·dx − b·v)·dt          damped Euler integration
 *  x += v·dt                     position update
 *  b ≈ 2√(k·m)                   critical damping (m=1 implicit)
 *  digit width ≈ height × 1.1    7-segment aspect inverted by 2:1 cell
 *  segment mask: bit s of k_segs[d]  → segment s lit on digit d
 *
 * EDGE CASES TO WATCH
 * ───────────────────
 *  • Particle never re-assigns to a different segment — if a digit needs
 *    more lit segments than another, the same N_PER_SEG=24 particles light
 *    each. Total lit dots is therefore digit-dependent (digit 1 = 48,
 *    digit 8 = 168).
 *  • g_dw clamps to g_cols-4; on tiny terminals (cols<10) the digit can
 *    collapse to width 6 and segments overlap visually.
 *  • Idle particles use JITTER random kicks — without damping they would
 *    diffuse off-screen; DRIFT_DAMP=1.5 keeps them bound.
 *  • dist² thresholds (0.64, 6.0) are squared for cheap compare; changing
 *    units accidentally is a classic bug.
 *  • lcg_f() is reseeded from clock_ns at start; restart shows different
 *    initial scatter but identical morph after first digit_assign.
 *
 * HOW TO VERIFY
 * ─────────────
 *  • Digit 1 should light only segments B and C (right column) — count
 *    visible '|' and confirm ≈48 of them.
 *  • Digit 8 should light all seven segments — particle count visible
 *    should approach 168.
 *  • Pause mid-morph; particles should freeze (no spring update).
 *  • After several seconds on the same digit, the lit segments should be
 *    perfectly straight rows of '─'/'│' (overdamped settling complete).
 *  • Cycling theme via t/T must repaint without visual jumps — only the
 *    colour pairs change, particle positions persist.
 *
 * ─────────────────────────────────────────────────────────────────────── */
