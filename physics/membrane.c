 ...
 *
 *  ┌──────────────────────────────────────────────────────────────────┐
 *  │ membrane  60.0 fps  sim:60 Hz  c=25  g=0.003  BC=Dirichlet  …    │ ← row 0   STATUS HUD
 *  │   . . . . . . . . . . . . . . . . . . . . . .                    │           (bright yellow + bold)
 *  │   . .            ████████                . . .                   │
 *  │   . .       ████████████████████         . .                     │
 *  │   . .    ██████████████████████████      . .                     │ ← cells colored by displacement u(x,y,t):
 *  │   . .    ████████  · · · · · · ░░░░      . .                     │     ⊕  warm  (red/orange/yellow/white) = crest (u > 0)
 *  │   . .    ▓▓░░░░  · · · · · · · · ░▒▒     . .                     │     ⊖  cool  (blue/cyan/violet)         = trough (u < 0)
 *  │   . .    ░░░░ · · · · · · · · · · ░░     . .                     │     brightness ∝ |u| / max_amplitude
 *  │   . .       · · · · · · · · · · · ·      . .                     │
 *  │   . .         ▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒            . .                     │ ← faint '·' on dim gray = NODAL LINES
 *  │   . .             ▓▓▓▓▓▓▓                . .                     │     (zero-crossing curves where u changes sign;
 *  │   . . . . . . . . . . . . . . . . . . . . . .                    │      the geometry of the standing-wave mode)
 *  │ +--- MEMBRANE WAVE -------+                                      │
 *  │ | max_amp   0.94          |                                      │ ← bottom-left overlay panel:
 *  │ | energy    0.012         |                                      │     numerical readouts of the wave's state
 *  │ | mode     (1, 1)         |                                      │     and the CFL stability indicator
 *  │ | CFL_2D   0.589 STABLE   |                                      │     (green=STABLE, yellow=MARGINAL, red=UNSTABLE)
 *  │ | …                       |                                      │
 *  │ q:quit  spc:pause  s:step  r:reset  b:center  e:edge  m:mode  …  │ ← row n-1 ACTION-KEYS HUD
 *  └──────────────────────────────────────────────────────────────────┘           (bright cyan + bold)
 *
...
#define DISPLAY_MAX_FLOOR   0.05f  /* min normaliser — keeps inv_max finite
...
#define CDIFF_FACTOR        0.5f   /* centred-difference: (f[i+1] - f[i-1])
...
#define STRIKE_X_LEFT_EDGE  0.15f  /* 15% in from the left wall —
...
#define STRIKE_X_DOUBLE_R   0.70f  /* DOUBLE preset, right strike —
                                    * 0.30 and 0.70 are symmetric about
                                    * the centre, but the y-offsets        */
#define STRIKE_Y_DOUBLE_R   0.65f  /* (0.35, 0.65) make the line through
...
static void apply_dirichlet_bc(float (*u)[GRID_MAX_W],
                               float (*v)[GRID_MAX_W],
                               int cols, int rows)
{
    for (int c = 0; c < cols; c++) {
        u[0][c]      = 0.0f;  v[0][c]      = 0.0f;
        u[rows-1][c] = 0.0f;  v[rows-1][c] = 0.0f;
    }
    for (int r = 0; r < rows; r++) {
        u[r][0]      = 0.0f;  v[r][0]      = 0.0f;
        u[r][cols-1] = 0.0f;  v[r][cols-1] = 0.0f;
    }
}
...
static void apply_neumann_bc(float (*u)[GRID_MAX_W],
                             float (*v)[GRID_MAX_W],
                             int cols, int rows)
{
    for (int c = 0; c < cols; c++) {
        u[0][c]      = u[1][c];      v[0][c]      = v[1][c];
        u[rows-1][c] = u[rows-2][c]; v[rows-1][c] = v[rows-2][c];
    }
    for (int r = 0; r < rows; r++) {
        u[r][0]      = u[r][1];      v[r][0]      = v[r][1];
        u[r][cols-1] = u[r][cols-2]; v[r][cols-1] = v[r][cols-2];
    }
}
...
static void apply_periodic_bc(float (*u)[GRID_MAX_W],
                              float (*v)[GRID_MAX_W],
                              int cols, int rows)
{
    for (int c = 0; c < cols; c++) {
        u[0][c]      = u[rows-2][c]; v[0][c]      = v[rows-2][c];
        u[rows-1][c] = u[1][c];      v[rows-1][c] = v[1][c];
    }
    for (int r = 0; r < rows; r++) {
        u[r][0]      = u[r][cols-2]; v[r][0]      = v[r][cols-2];
        u[r][cols-1] = u[r][1];      v[r][cols-1] = v[r][1];
    }
}
...
static void apply_bc(int bc)
{
    ...
    switch (bc) {
    case BC_DIRICHLET: apply_dirichlet_bc(u, v, cols, rows); break;
    case BC_NEUMANN:   apply_neumann_bc  (u, v, cols, rows); break;
    case BC_PERIODIC:  apply_periodic_bc (u, v, cols, rows); break;
    }
}
...
static void velocity_half_step(float (*u)[GRID_MAX_W],
                               float (*v)[GRID_MAX_W],
                               int cols, int rows,
                               float c2, float damping, float dt)
{
...
static void scan_peak_and_energy(const float (*u)[GRID_MAX_W],
                                 const float (*v)[GRID_MAX_W],
                                 int cols, int rows, float c2,
                                 float *out_max_amp, double *out_ke,
                                 double *out_pe)
{
    float  mx = 0.0f;
...
static void estimate_mode_numbers(const float (*u)[GRID_MAX_W],
                                  int cols, int rows,
                                  int *out_nx, int *out_ny)
{
    ...
    for (int c = 1; c < cols - 1; c++)
        if (u[cr][c-1] * u[cr][c] < 0.0f) crossings_x++;
    for (int r = 1; r < rows - 1; r++)
        if (u[r-1][cc] * u[r][cc] < 0.0f) crossings_y++;
                          ...
                          float *cfl_2d)
{
    ...
    double ke, pe;
    scan_peak_and_energy(ufld, vfld, cols, rows, c2,
                         max_amplitude, &ke, &pe);
...
static void mode_wavenumbers(int nx, int ny, int cols, int rows,
...
{
    if (fabsf(u[r][c]) >= near_zero_thresh) return false;
...
static void paint_amplitude_cell(WINDOW *w, int r, int c,
                                 float u, int theme, float inv_max)
{
    ...
    if (lvl == 0) return;        /* below display threshold — skip */
...
static void render_membrane(WINDOW *w,
                            ...
                            float display_max)
{
    ...
    if (display_max < DISPLAY_MAX_FLOOR) display_max = DISPLAY_MAX_FLOOR;
...
static void scene_draw(const Scene *s, WINDOW *w, float alpha, float dt_sec)
{
    ...
    render_overlay(w, g_grid.cols, g_grid.rows,
                   s->max_amplitude, s->energy_est,
                   s->mode_nx, s->mode_ny,
...
{
    int idx = s->mode_idx;
    if (idx < 0 || idx >= MODE_TABLE_LEN) idx = 0;
    preset_resonance(k_mode_table[idx][0],
                     k_mode_table[idx][1],
                     RESONANCE_AMP);
    apply_bc(bc);
}
...
static void preset_apply(Scene *s, int id)
{
    ...
    switch (id) {
    case PRESET_CENTER:    strike_centre_pulse                 (s->bc); break;
    case PRESET_EDGE:      strike_left_edge_pulse              (s->bc); break;
    case PRESET_DOUBLE:    strike_double_offcentre_pulses      (s->bc); break;
    case PRESET_RESONANCE: strike_next_standing_wave_mode  (s, s->bc); break;
    }
}
...
static bool app_handle_key(App *app, int ch)
{
    ...
    case 'b': case 'B':
        ...
        apply_excitation((float)(g_grid.cols-1)*0.5f,
                         (float)(g_grid.rows-1)*0.5f,
                         EXCITE_AMP, EXCITE_RADIUS);
        apply_bc(s->bc);
        break;
    ...
    case 'f': case 'F':
        apply_excitation((float)(g_grid.cols-1)*0.30f,
                         (float)(g_grid.rows-1)*0.35f,
                         EXCITE_AMP, EXCITE_RADIUS);
        apply_excitation((float)(g_grid.cols-1)*0.70f,
                         (float)(g_grid.rows-1)*0.65f,
                         EXCITE_AMP, EXCITE_RADIUS);
        apply_bc(s->bc);
        break;
