...
#define DEPTH_FAR_SCALE 2.2f    /* depth mode: t_max = cam_dist * this      */
...
/* ── §5 LOGIC — ray-sphere intersection (THE CORE), pure
 * ───────────────────────────── */
...
/*
 * ShadeMode — which shading model §7 runs for every ray that hits the sphere.
 * The geometry (ray_sphere → P, N, V_dir) is IDENTICAL across modes; only the
 * surface-colour function changes — that is the whole pedagogical point: one
 * intersection feeds four different "what does this surface look like?"
 ...
 * light_pos is a WORLD position; the direction L = norm(light_pos − P) is
 * recomputed per hit (point light). spec_gain = 0 gives a pure diffuse fill.
 * Ref: Phong 1975 (specular reflection model). */
...
/* shade_pixel — dispatch one sphere hit to the active shading model, returning
 * its display colour and (via *lum) the luminance that picks the ramp glyph.
 * The geometry (P, N, V_dir, t_hit) is mode-independent; only the colour /
 * luma rule differs — the demo's core lesson (one intersection, four looks). */
...
/* ── SIMULATION STATE — Camera + Scene (read by §7 render, driven by §9) ─ *
 *
 * Camera — the orbiting viewpoint. The sphere is fixed at the origin; the
 * camera circles it, so its persistent state is just the orbit angle (which
 * advances each tick) and the orbit radius (the user's zoom knob). render
 * (§7) rebuilds the eye position and the (fwd,right,up) basis from these two
 * numbers every frame — see THE ORBITING-CAMERA TRICK there.
 * Ref: look-at camera basis (orthonormalised via fwd × world-up, Gram-
 * Schmidt); polar orbit parameterization eye = (r·sinθ, h, −r·cosθ).
 *
 * Scene — the whole demo's persistent mutable state in one place: the table
 * of contents the loop in §9 drives.
 *   WHAT : camera          — the orbiting viewpoint (angle advances; dist=zoom)
 *   HOW  : paused          — sim knob: freeze the orbit advance (DELAYS gate)
 *          theme_idx, mode — render knobs: which material + which shading model
 * (No WHERE struct: run/resize flags are signal-written globals in §9, and the
 *  frame timer is loop-local in main — neither is a reusable concept here.)
 * ─────────────────────────────────────────────────────────────────────── */

typedef struct {
  float orbit_ang; /* current orbit angle (rad); advances each tick    */
  float dist;      /* orbit radius from origin (world units); zoom knob */
} Camera;

typedef struct {
  Camera camera;  /* WHAT: the orbiting viewpoint (angle + zoom radius)   */
  int paused;     /* HOW:  freeze the orbit advance (the DELAYS gate)     */
  int theme_idx;  /* HOW (render): active material, index into g_themes   */
  ShadeMode mode; /* HOW (render): active shading model                   */
} Scene;

/* scene_init — boot state: gold material, phong shading, default zoom, orbit
 * at angle 0, running (not paused). The ONLY Scene initialiser. */
 ...
 * nanoseconds: rotate the camera's orbit angle, unless paused. The ONLY
 * function that advances simulation state (§9.3 calls it once per frame). */
 ...
 *
 * Inputs:
 *   s           Scene (read-only): camera orbit (angle + radius), active
 *               theme_idx, and shading mode
 ...
 *     V_dir = norm(cam − P)
 *     color = shade_pixel(mode, P, N, V_dir, t, dist, th, &lum)
...
static void render(const Scene *s, int cols, int rows) {
  ...
  for (int row = 0; row < rows - 1; row++) {
    for (int col = 0; col < cols; col++) {
      ...
      float lum;
      V3 color =
          shade_pixel(s->mode, P, N, V_dir, t_hit, s->camera.dist, th, &lum);
      draw_color(row, col, color, lum);
    }
  }
}
...
static void hud_draw(const Scene *s, float fps, int cols, int rows) {
  ...
  snprintf(buf, sizeof buf, " %5.1f fps  dist:%.1f  %-9s  %s ", (double)fps,
           (double)s->camera.dist, g_themes[s->theme_idx % THEME_N].name,
  ...
  mvprintw(rows - 1, 0, " q:quit  spc/p:pause  s:mode  t:theme  +/-:zoom ");
...
int main(void) {
  ...
  while (g_run) {
    ...
    /* §9.3 SIMULATION (the tick) — scene_tick advances the orbit; paused =
     * DELAYS. */
    scene_tick(&scene, dt);
    ...
    /* §9.6 USER EVENTS — drain key input (knobs / quit; NOT part of the tick).
     */
    int ch = getch();
    switch (ch) {
    case 'q':
    ...
    case '+':
    case '=':
      scene.camera.dist -= CAM_DIST_STEP;
      if (scene.camera.dist < CAM_DIST_MIN)
        scene.camera.dist = CAM_DIST_MIN;
      break;
    case '-':
    case '_':
      scene.camera.dist += CAM_DIST_STEP;
      if (scene.camera.dist > CAM_DIST_MAX)
        scene.camera.dist = CAM_DIST_MAX;
      break;
    default:
      break;
    }
 ...
 *   §1 config     — [DATA]   frame rate, FOV, geometry, camera, ramp, HUD
 *   §2 clock      — [PERF]   monotonic timer + sleep
 *   §3 math       — [LOGIC]  V3 helpers (pure)
 *   §4 color      — [RENDER+DATA] themes + 256-colour cube + ramp painter
 *   §5 sphere     — [LOGIC]  THE CORE: analytic ray-sphere quadratic (pure)
 *   §6 shading    — [LOGIC]  phong / normals / fresnel / depth (pure)
 *                   add_phong_light → shade_phong (§6.1: KEY/FILL/RIM lights)
 *                   shade_normal (§6.5)  shade_fresnel + shade_depth (§6.6)
 *                   shade_pixel — picks the shading model per ray
 *   §7 render     — [RENDER] one frame: ray-per-cell → shade_pixel
 *   §8 screen     — [RENDER] ncurses init + HUD
 *   §9 app        — [SIM+APP] tick (orbit), events, performance, main loop
 *
 * Keys:
 ...
 * (camera + knobs) plus a few timing locals in main() (§9), see MUTATES.
 *
 *   LAYER        SECTION(S)        MUTATES
 *   ----------   ---------------   -----------------------------------------
 *   SIMULATION   §9.3 scene_tick   scene.camera.orbit_ang
 *   LOGIC        §3 math,          nothing — pure functions; same inputs
 *                §5 ray-sphere,    give the same V3/float/int, so render or
 *                §6 shading        effects ordering cannot corrupt them
 ...
 *   3. SIM      scene_tick: orbit_ang += SPEED·dt       §9.3  ← the tick
 ...
 *
 * User EVENTS (resize §9.1, keys §9.6) may change knobs (scene.camera.dist,
 * scene.theme_idx/.mode/.paused) or quit, but are NOT the tick — they set the
 * inputs the next tick
 * and render read. There is no reset: the only simulation state is the
 * freely-running orbit angle, which has nothing to reset to.
 * ─────────────────────────────────────────────────────────────────── */
