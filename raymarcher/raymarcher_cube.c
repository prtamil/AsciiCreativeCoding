 ...
 */
static float sphere_trace(Vec3 ro, Vec3 rd, float rx, float ry,
                          float cube_half, int *out_steps)
{
    float t = RM_T_START;
    int   step;
...
static Hit cast_ray(int col, int row, int cw, int ch,
                    float rx, float ry, float cube_half,
                          ...
                          Vec3 light, float cam_z)
{
    for (int row = 0; row < c->h; row++) {
        for (int col = 0; col < c->w; col++) {
            c->hits[row * c->w + col] =
                cast_ray(col, row, c->w, c->h,
                         rx, ry, cube_half, light, cam_z);
        }
    }
}
...
static int face_slot_for_normal(Vec3 N)
{
    ...
    if (ax >= ay && ax >= az) return (N.x > 0) ? 7 : 6;
    if (ay >= az)             return (N.y > 0) ? 5 : 4;
    return (N.z > 0) ? 2 : 1;
}
...
static void scene_render(Scene *s)
{
    canvas_render(&s->canvas,
                  s->rot_angle_x, s->rot_angle_y, s->cube_half,
...
static bool app_handle_key(App *app, int ch)
{
    ...
    case ']':
        s->spin_speed_x *= ROT_STEP;   s->spin_speed_y *= ROT_STEP;
        if (s->spin_speed_x > ROT_MAX) s->spin_speed_x = ROT_MAX;
        if (s->spin_speed_y > ROT_MAX) s->spin_speed_y = ROT_MAX;
        break;
    case '[':
        s->spin_speed_x /= ROT_STEP;   s->spin_speed_y /= ROT_STEP;
        if (s->spin_speed_x < ROT_MIN) s->spin_speed_x = ROT_MIN;
        if (s->spin_speed_y < ROT_MIN) s->spin_speed_y = ROT_MIN;
        break;

    case '=': case '+':
        s->cube_half *= SIZE_STEP;
        if (s->cube_half > SIZE_MAXX) s->cube_half = SIZE_MAXX;
        break;
    case '-':
        s->cube_half /= SIZE_STEP;
        if (s->cube_half < SIZE_MIN) s->cube_half = SIZE_MIN;
        break;
