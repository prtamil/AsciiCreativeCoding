...
static void draw_line_supercover(WINDOW *w,
                                ...
                                bool *used)
{
    ...
    /* helper macro: safe draw with stipple + grid lock */
#define TRY_DRAW(cx, cy, ch)                                       \
    do {                                                           \
        if ((cx) >= 0 && (cx) < cols &&                            \
            (cy) >= 0 && (cy) < rows &&                            \
            step % stipple == 0 &&                                 \
    ...
    if (dx >= dy) {
        ...
        while (x != x1) {
            ...
            if (err < 0) {
                ...
                /*
                 supercover correction:
                 if we crossed a boundary corner,
                 draw the neighbouring cell too
                */
                if (prev_err < dy) {
                    TRY_DRAW(x, y + sy, diag);
                }
            }
        ...
        while (y != y1) {
            ...
            if (err < 0) {
                ...
                /*
                 supercover correction:
                 fill adjacent boundary-crossed cell
                */
                if (prev_err < dx) {
                    TRY_DRAW(x + sx, y, diag);
                }
            }
