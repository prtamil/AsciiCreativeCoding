...
#define CELL_H   16
#define N_DISCS   25
#define R_MIN    (CELL_W * 1.5f)   /* min radius in pixels  */
#define R_MAX    (CELL_W * 4.0f)   /* max radius in pixels  */
#define V_MAX     180.f             /* initial speed px/s    */
#define FLASH_S    0.4f             /* impact flash duration */
...
static void disc_tick(float dt)
{
    ...
    for (int i = 0; i < N_DISCS - 1; i++) {
        for (int j = i + 1; j < N_DISCS; j++) {
            ...
            if (dv <= 0.f) continue;  /* already separating */

            float imp = 2.f * ma * mb / total * dv;
            a->vx -= imp / ma * nx;
            a->vy -= imp / ma * ny;
            b->vx += imp / mb * nx;
            b->vy += imp / mb * ny;
...
static void scene_draw(void)
{
    for (int i = 0; i < N_DISCS; i++) {
        ...
        int cp;
        if (d->flash > 0.f)       cp = CP_FLASH;
        else if (speed < V_MAX*.4f) cp = CP_SLOW;
        else if (speed < V_MAX*.8f) cp = CP_MED;
        else                        cp = CP_FAST;
        draw_disc(d, cp);
    }
