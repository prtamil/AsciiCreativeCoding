...
static void pool_add(ObjectPool *p, int Q, int R, int sector, char glyph)
{
...
static void pool_draw(const ObjectPool *p, double size, int ox, int oy, int rows, int cols)
{
    ...
    for (int i = 0; i < p->count; i++) {
        int sc, sr;
        wedge_to_screen(p->objs[i].Q, p->objs[i].R, p->objs[i].sector, size, ox, oy, &sc, &sr);
...
typedef struct { int Q, R, sector; double hex_size; int glyph_idx, theme, paused; } Cursor;
...
/* STAR: cursor's 6 sectors + 6 outer hex sector-3 */
                          ...
                          int cQ, int cR, char glyph)
{
    for (int i = 0; !IS_END(pat[i]); i++)
        pool_add(pool, cQ + pat[i][0], cR + pat[i][1], pat[i][2], glyph);
}
...
static void pattern_scatter(ObjectPool *pool, int cQ, int cR, char glyph)
{
    ...
    while (n > 0 && tries < 100) {
        ...
        int s  = (int)(frand() * 6);
        int prev = pool->count;
        pool_add(pool, cQ + dQ, cR + dR, s, glyph);
