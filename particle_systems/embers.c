 ...
 *   d / D      next / previous debug overlay  (normal / velocity / temperature
 * / source)
 *
 * Build:
 ...
 * — up) ember.vx   += turbulence · (r-0.5) · dt ember.x    += ember.vx · dt
 *                         ember.y    += ember.vy · dt
 *                         ember.age  += dt
 *
 ...
 *     idea, smaller scale.
 *   • Wikipedia — [Black-body
 * radiation](https://en.wikipedia.org/wiki/Black-body_radiation). Real embers
 * cool through a temperature-driven colour sequence
...
/* ember_advance_one — full per-ember frame: turbulence + buoyancy + integrate +
 * death. */
...
/* topup_ember_pool — phase 1: spawn embers up to pp->target_embers
 * (rate-capped). */
 ...
 *   scene_draw                  phase 1 then phase 2; const Scene * (no
 * mutation)
 *
...
/* source_band_slot_for_dx — bright at centre (slot 7), dimmer toward edges;
 * floor 4. */
static inline int source_band_slot_for_dx(int dx, int half) {
  float r = (float)abs(dx) / (float)(half + 1);
  int slot = (int)((1.0f - r) * 7.0f + 0.5f);
  if (slot < 4)
    slot = 4; /* always reasonably hot   */
  if (slot > 7)
    slot = 7;
  return slot;
}

/* draw_source_band — phase 1: 2-row glow strip at the source so fire reads
 * anchored. */
