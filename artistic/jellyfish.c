 ...
 * CORE IDEA
 * ─────────
 * One jellyfish is a finite-state machine driving a half-ellipse plus
 * three wavy chains.  The four states are IDLE → CONTRACT → GLIDE →
 * EXPAND → IDLE, executed once per "pulse".  Two physical scalars carry
 * the whole locomotion: bell_open ∈ [MIN_OPEN, 1] (visual) and vy
 * (vertical velocity, with gravity always pulling +y and a JET_THRUST
 * impulse fired at the end of CONTRACT).  Drawing reads bell_open to
 * scale the ellipse; the tentacles trail with a velocity-proportional
 * lag so a fast-rising bell drags its tentacles like ribbons.
 *
 * HOW TO THINK ABOUT IT
 * ─────────────────────
 * Picture a slow blink of a one-eyed creature whose eyelid does the
 * propulsion: open (fall), close fast (push water down → rise), wait
 * (coast), open slow (settle), repeat.  The bell is just an animated
 * eyelid in cross-section.  Tentacles are ribbons tied to the underside
 * — they sway because the water shears past them, except during a
 * sudden pulse when they momentarily streak straight behind.  Each
 * jellyfish runs independently because every spawn randomises its
 * initial anim_t and idle_dur.
 *
 * ALGORITHM IN STEPS
 * ──────────────────
 *  1. Each tick (dt seconds), per jellyfish:
 *       a. anim_t += dt; advance the state machine.
 *          IDLE     : bell_open = 1; vy += GRAVITY · dt; transition
 *                     after idle_dur seconds.
 *          CONTRACT : bell_open eases from 1 to MIN_OPEN over
 *                     DUR_CONTRACT (curve_contract = 1 - (1-t)²);
 *                     at the end, vy = -JET_THRUST.
 *          GLIDE    : bell stays closed for DUR_GLIDE, momentum coasts.
 *          EXPAND   : bell_open eases from MIN_OPEN to 1 over DUR_EXPAND
 *                     (smoothstep), wave_scale ramps back to 1.
 *       b. Apply drag every frame: vy *= exp(-DRAG_VY · dt).
 *       c. drift_phase += 2π · DRIFT_HZ · dt; cx oscillates by
 *          drift_cx + DRIFT_A · sin(drift_phase).
 *       d. cy += vy · dt.  Wrap when fully above screen.
 *  2. Draw bell: scan rows, for each row compute ny = (py - cy)/Ry;
 *     half-ellipse equation gives nx_edge = sqrt(1 - ny²), thus left/
 *     right column.  Use crown_ny_limit = 0.88 + 0.12·crown_f to clip
 *     the apex when contracted.
 *  3. Pick edge glyphs by abs_ny: > 0.94 '_' roof, > 0.88 '(_)' crown,
 *     > 0.40 '/\\', > 0.14 '()', else '~'.  Inside the dome plant '.'
 *     translucent dots where the radial test r² > 0.60.
 *  4. Draw N_TENTACLES chains from base_x = cx + (k - 1) · Rx.  Each
 *     segment seg has a sinusoidal lateral offset and an inertia lag
 *     of -vy · TENT_LAG · depth (tip lags more than root).  Glyph by
 *     depth: |, |, :, .
 *
 * KEY FORMULAS
 * ────────────
 *  Ease-out         curve_contract(t) = 1 - (1 - t)²     fast then slow
 *  Smoothstep       curve_expand(t)   = t² (3 - 2t)      slow start/end
 *  Bell open        bell_open ∈ [MIN_OPEN, 1] (driven by curve)
 *  Crown apex       crown_ny_limit = 0.88 + 0.12 · crown_f
 *  Half ellipse     nx_edge = sqrt(1 - ny²),  Rx = BELL_RX_BASE · head_f
 *  Gravity          vy += GRAVITY · dt        (always +y = downward)
 *  Jet thrust       vy ← -JET_THRUST          (negative = upward kick)
 *  Drag             vy ← vy · exp(-DRAG_VY · dt)
 *  Drift            cx = drift_cx + DRIFT_A · sin(drift_phase)
 *  Tentacle wave    lat = AMP · wave_scale · sin(phase - seg·K + k·OFF)
 *  Inertia lag      lag = -vy · TENT_LAG · depth   (depth ∈ [0,1])
 *  Wrap             if cy + tentacle_extent < 0  → respawn at bottom
 *
 * EDGE CASES TO WATCH
 * ───────────────────
 *  • MIN_OPEN=0.85 is a tight squeeze; pushing it below 0.5 makes the
 *    bell flat (apex below cy) and the rim row vanishes off-screen.
 *  • The exp(-DRAG_VY · dt) integration is exact (analytic decay), not
 *    Euler — increasing DRAG_VY past ~5 makes vy reach near-zero in one
 *    frame and the GLIDE phase becomes invisible.
 *  • TENT_LAG · vy can push tip rows below the screen; segments are
 *    bounds-checked but the bell appears to "lose" its tail at peak
 *    upward velocity — this is the inertia look, not a bug.
 *  • Resize updates max_px / max_py but does NOT respawn; the existing
 *    jellyfish may end up clipped against the new edges.  Pressing 'r'
 *    fully respawns.
 *  • Adding the 8th jellyfish puts it in a randomised zone — visual
 *    overlap is possible.  At default radii two close ones can paint
 *    the same row.
 *  • The tentacle phase advances at TENT_WAVE_SPD · wave_scale · dt;
 *    when wave_scale falls to 0.12 during glide the phase nearly
 *    freezes — tentacles stiffen.
 *  • randf() uses rand() which is not seeded between jellyfish, only
 *    once at startup; identical seeds produce identical trajectories.
 *
 * HOW TO VERIFY
 * ─────────────
 *  • Pulse cadence: IDLE_DUR_MIN..MAX + DUR_CONTRACT + DUR_GLIDE +
 *    DUR_EXPAND ≈ 0.55–1.10 + 0.19 + 0.38 + 0.68 = 1.80–2.35 s per
 *    full cycle.  Time one pulse with a stopwatch.
 *  • Pause (space) and confirm bell freezes mid-state — anim_t stops
 *    accumulating because scene_tick returns early.
 *  • Press '+' until 8 jellies; verify each gets its own colour pair
 *    (cycling through CP_J0…CP_J7).  Press '-' to remove the most
 *    recent.
 *  • Watch CONTRACT: bell snaps fast (ease-out), then GLIDE the bell
 *    is fully closed for ~0.4 s, then EXPAND opens slowly.
 *  • A jellyfish that drifts off the top wraps to the bottom; visible
 *    re-entry confirms the cy + tentacle_extent < 0 wrap.
 *
 * ─────────────────────────────────────────────────────────────────────── */
