 ...
 *                       threshold, the spark is killed (it would
 *                       otherwise micro-bounce forever).
 *
...
static const char *pattern_name(Pattern p)
{
    ...
    case PATTERN_SPARKLERS:      return "SPARKLERS";
    case PATTERN_GROUND_SPINNER: return "SPINNER  ";
    case PATTERN_FLOWER_POT:     return "FLOWERPOT";
    case PATTERN_CHRYSANTHEMUM:  return "CHRYSANTH";
    case PATTERN_WILLOW:         return "WILLOW   ";
...
typedef enum {
    ...
    EMIT_CENTER_BOTTOM,  /* x = cols/2,   y = rows-4  (GRINDER, CAMPFIRE, FLOWERPOT, WILLOW)   */
    EMIT_CENTER,         /* x = cols/2,   y = rows/2  (TESLA, SPARKLERS, SPINNER, CHRYSANTH)   */
...
static const PatternParams pattern_params[N_PATTERNS] = {
     ...
     */
    /* WELDING       */ { 380, EMIT_CENTER_LEFT,   1.0f, 1.0f,   55.0f,  90.0f,  -0.55f,                 0.55f,                  78.0f, 0.40f, 0.55f, 0.78f, 1.0f, 2.2f },
    /* GRINDER       */ { 320, EMIT_CENTER_BOTTOM, 0.6f, 0.6f,   72.0f, 110.0f,  -1.30f,                -0.40f,                  92.0f, 0.30f, 0.50f, 0.70f, 0.8f, 1.8f },
    /* CAMPFIRE      */ { 220, EMIT_CENTER_BOTTOM, 3.0f, 0.5f,   34.0f,  56.0f,  -2.20f,                -0.94f,                  36.0f, 0.55f, 0.40f, 0.60f, 1.5f, 3.0f },
    /* TESLA         */ { 260, EMIT_CENTER,        0.4f, 0.4f,   55.0f,  85.0f,  -(float)M_PI,           (float)M_PI,            26.0f, 0.20f, 0.65f, 0.78f, 0.5f, 1.2f },
    /* SPARKLERS     */ { 200, EMIT_CENTER,        0.3f, 0.3f,   28.0f,  50.0f,  -(float)M_PI,           (float)M_PI,            60.0f, 0.50f, 0.35f, 0.65f, 0.4f, 1.0f },
    /* SPINNER       */ { 380, EMIT_CENTER,        0.5f, 0.5f,   90.0f, 130.0f,  -(float)M_PI,           (float)M_PI,            50.0f, 0.30f, 0.55f, 0.75f, 1.0f, 2.0f },
    /* FLOWERPOT     */ { 400, EMIT_CENTER_BOTTOM, 0.5f, 0.5f,  100.0f, 150.0f,  -(float)(M_PI/2+0.6),  -(float)(M_PI/2-0.6),    70.0f, 0.25f, 0.45f, 0.70f, 2.0f, 3.5f },
    /* CHRYSANTHEMUM */ { 320, EMIT_CENTER,        0.5f, 0.5f,   50.0f,  75.0f,  -(float)M_PI,           (float)M_PI,            18.0f, 0.18f, 0.40f, 0.65f, 3.0f, 4.5f },
    /* WILLOW        */ { 280, EMIT_CENTER_BOTTOM, 0.5f, 0.5f,   50.0f,  80.0f,  -(float)(M_PI/2+1.2),  -(float)(M_PI/2-1.2),   100.0f, 0.15f, 0.30f, 0.60f, 2.8f, 4.5f },
    /* WATERFALL     */ { 380, EMIT_CENTER_TOP,    0.5f, 0.5f,   35.0f,  65.0f,   (float)(M_PI/2-0.35),  (float)(M_PI/2+0.35),   45.0f, 0.25f, 0.65f, 0.78f, 1.8f, 3.0f },
};
...
static const Theme themes[N_THEMES] = {
    ...
    { "ECLIPSE",  {  53,  54,  89, 125, 161, 197, 204, 209 } },  /* void purple→crimson   */
};
...
static inline void velocity_from_polar(float speed, float angle,
                                       float *vx, float *vy)
{
    *vx = speed * cosf(angle);
    *vy = speed * sinf(angle);
...
static inline void integrate_spark_semi_implicit_euler(Spark *e,
                                                       float gravity,
                                                       float drag_factor,
                                                       float dt)
{
...
static inline bool spark_crossed_floor_descending(const Spark *e, float floor_y)
{
...
static inline bool spark_should_die(const Spark *e, int cols)
{
    ...
    if (e->y < -TOP_KILL_MARGIN)                      return true;
    return false;
}
...
static void scene_spawn_spark(Scene *s)
{
    ...
    if (s->current_pattern == PATTERN_GROUND_SPINNER)
        ...
        cone_compute_emission(&s->rng, pp, cx, cy,
                              &spawn_x, &spawn_y, &emit_angle);
...
{
    int idx = spark_pool_find_inactive(s);
    if (idx < 0) return;     /* pool full — accept that some children fail */
    Spark *e = &s->sparks[idx];

    float angle = sample_random_phase_2pi(&s->rng);
    float speed = sample_uniform_in_range(&s->rng,
                                          SPARKLER_CHILD_SPEED_MIN,
                                          SPARKLER_CHILD_SPEED_MAX);
    float life  = sample_uniform_in_range(&s->rng,
                                          SPARKLER_CHILD_LIFE_MIN,
...
static int count_active_parent_sparks(const Scene *s)
{
    int n = 0;
    for (int i = 0; i < MAX_SPARKS; i++)
        if (s->sparks[i].active && !s->sparks[i].has_split) n++;
    return n;
}
...
{
    for (int k = 0; k < n; k++) scene_spawn_spark(s);
}
...
static void cascade_sparkler_split_pass(Scene *s)
{
    for (int i = 0; i < MAX_SPARKS; i++) {
        Spark *e = &s->sparks[i];
        if (!e->active || e->has_split)                 continue;
        if (e->age < e->life * SPARKLER_SPLIT_AGE_FRAC) continue;
        e->has_split = true;
...
static void scene_tick(Scene *s, float dt)
{
    ...
    for (int i = 0; i < MAX_SPARKS; i++) {
        ...
        if (spark_should_die(e, s->cols)) e->active = false;
    }
...
static void trail_draw_one_spark(const Spark *e, int head_slot,
                                 int cols, int rows_eff)
{
    for (int k = 0; k < TRAIL_LEN; k++) {
        int slot = trail_ramp_slot_for_offset(head_slot, k);
        if (slot < 0) continue;                 /* cooled below the ramp */
